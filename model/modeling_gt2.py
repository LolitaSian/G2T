...
class BartStructureEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        # mbart has one extra layer_norm
        self.layer_norm = LayerNorm(config.d_model) if config.normalize_before else None
        self.gat_encoder = GAT(embed_dim, embed_dim, embed_dim, 0.2, 0.2, 1) #init gat
        self.dim_adjust_linear = nn.Linear((config.max_node_length + 1), config.max_input_length)
        self.dim_adjust_linear2 = nn.Linear(config.d_model * 2, config.d_model)
        #fusion module
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=config.d_model * 2, nhead=2)
        self.alignment_module = nn.TransformerEncoder(self.encoder_layer, num_layers=2)
    ...
    def forward(self, input_ids, attention_mask=None, output_attentions=False, output_hidden_states=False,
        ...
        for encoder_layer in self.layers:
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                x, attn, x_node = encoder_layer(x, attention_mask, output_attentions=output_attentions, \
        ...
        for bid in range(bsz):
            gat_node = x_node[bid]
            gat_adj = adj_matrix[bid]
            output = self.gat_encoder(gat_node, gat_adj)
            gat_out.append(output)
        x_node_gat = torch.stack(gat_out, 0) #batch_size, nodes, embeding_dim
        x_node_gat = x_node_gat.transpose(1,2)
        x_node_gat = self.dim_adjust_linear(x_node_gat)
        x_node_gat = x_node_gat.transpose(1,2)
        ...
        x = x.transpose(0, 1)
        # residual_x = x
        x_node_gat = x_node_gat*0.1
        x = torch.cat((x, x_node_gat), dim=-1)
        x = self.alignment_module(x)
        x = self.dim_adjust_linear2(x)
...
class SelfStructureAttention(nn.Module):
    ...
    def forward(
        ...
        rel_v = rel_v.contiguous().view(bsz * self.num_heads, tgt_len, -1, self.head_dim)  # (batch * head) * tgt_len * src_len * (embed_dim / head)
        ...
        if key_padding_mask is not None:  # don't attend t o padding symbols
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
            reshaped = key_padding_mask.unsqueeze(1).unsqueeze(2)
            attn_weights = attn_weights.masked_fill(reshaped, float("-inf"))
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
...
class GraphPointer(nn.Module):
    def __init__(self, embedding_size, hidden_size):
        ...
        self.proj_linear2 = nn.Linear(hidden_size, 1)
    ...
    def forward(self, embeddings, hidden_states, pointer):
        ...
        """
        embed = embeddings + hidden_states
        copy_prob = torch.sigmoid(self.proj_linear(embed))
        copy_prob = torch.sigmoid(self.proj_linear2(copy_prob)).squeeze(-1)
...
class MyBartForConditionalGeneration(BartForConditionalGeneration):
    def __init__(self, config):
        ...
        self.model = base_model
        self.copyer = GraphPointer(config.d_model, config.d_model)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))

    def forward(self, input_ids, attention_mask=None, encoder_outputs=None,
            decoder_input_ids=None, decoder_attention_mask=None, input_node_ids=None, input_edge_ids=None,
            node_length=None, edge_length=None, adj_matrix=None, decoder_whole_ids=None,encoder_label=None, decoder_cached_states=None,
            use_cache=False, is_training=False, use_copyer = False):
        ...
        if is_training:
            ...
            if use_copyer:
                decoder_input_embeddings = self.model.shared(_decoder_input_ids)
                decoder_output_hiddens = outputs[1][-1]

                device = decoder_attention_mask.device  # 获取 decoder_attention_mask 的设备
                true_tensor = torch.tensor(True, device=device)
                false_tensor = torch.tensor(False, device=device)
                pointer = torch.where(decoder_attention_mask != 0, true_tensor, false_tensor)
                copy_prob = self.copyer(decoder_input_embeddings, decoder_output_hiddens, pointer)
                copy_loss = copy_prob.masked_select(pointer).mean()
                return 0.7 * loss + 0.3 * copy_loss
            else:
                return loss
